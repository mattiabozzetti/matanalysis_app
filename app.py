...
/* Streamlit tabs: force neon/aqua selected tab instead of default red */
...
GENERAL_DEFINITIONS = [
    ("Raw", "Valore osservato nel dataset, senza correzioni per il contesto di possesso squadra."),
    ("Possession-adjusted", "Valore corretto con formula sigmoide in base al possesso squadra. Le metriche on-ball sono leggermente penalizzate in squadre ad alto possesso e premiate in squadre a basso possesso; le off-ball fanno il contrario."),
    ("Percentile", "Posizionamento 0–100 rispetto al gruppo di riferimento selezionato. 90 significa sopra circa il 90% dei pari ruolo/contesto."),
    ("Overall role-based", "Media pesata dei punteggi di famiglia. I pesi cambiano per ruolo: ad esempio un attaccante pesa di più final product e shooting, un difensore difesa, duelli e sicurezza."),
    ("Playing Style radar", "Radar su metriche di volume: descrive quanto spesso un giocatore compie certe azioni."),
    ("Performance radar", "Radar su metriche di qualità/successo: descrive quanto bene il giocatore esegue quelle azioni."),
    ("Fit Index", "Nella pagina Similarity misura quanto il profilo percentile del giocatore è compatibile con il profilo medio del campionato target."),
    ("League Weighting", "Aumenta il peso delle feature che caratterizzano maggiormente il campionato target rispetto al riferimento generale."),
]
...
DEFINITIONS: dict[str, str] = {
    ...
    "Scoring attack involvement": "Coinvolgimento in azioni d’attacco che terminano con gol o situazione di scoring rilevante.",
    "Actions in box": "Azioni effettuate nell’area di rigore avversaria.",
    "Shots": "Tiri tentati.",
    "Shots on target": "Tiri nello specchio della porta.",
    "Shots on target %": "Quota di tiri che finiscono nello specchio; misura di precisione.",
    "xG per shot": "Qualità media dei tiri: xG medio per conclusione.",
    "xG conversion": "Efficienza realizzativa rispetto agli xG.",
    "Shots in box": "Tiri dall’interno dell’area di rigore.",
    "Shots outside box": "Tiri da fuori area.",
    ...
    "Received in final third": "Passaggi ricevuti nell’ultimo terzo.",
    "Received in box": "Passaggi ricevuti nell’area avversaria.",
    ...
    "Entries via pass": "Ingressi nell’ultimo terzo tramite passaggio.",
    "Entries via carry": "Ingressi nell’ultimo terzo portando palla.",
    ...
    # GK
    "Goals prevented": "Differenza tra xG dei tiri affrontati e gol concessi: quanti gol il portiere evita rispetto all’atteso.",
    "Goals prevented %": "Goals prevented normalizzato sul volume/qualità dei tiri affrontati.",
    ...
    "Shots on target faced": "Tiri nello specchio affrontati. Volume di lavoro.",
    ...
    "Long distribution share": "Quota di distribuzioni lunghe sul totale.",
...
def metric_definition(metric: dict[str, Any]) -> str:
    ...
    if kind == "quality":
        return f"{column}. Indicatore di qualità/efficienza: percentuali o conversioni."
    if kind == "volume":
...
def kind_label(metric: dict[str, Any]) -> str:
    ...
    if metric.get("higher_is_better") is False or kind == "negative":
        return "negative"
    if kind in {"quality", "volume", "derived", "style"}:
        return kind
    return kind
...
def render_family(group_name: str, group: dict[str, Any]) -> None:
    ...
    parts = []
    parts.append(f'<div class="glossary-family" style="border-color:{color}38;">')
    parts.append('<div class="glossary-family-head">')
    parts.append(f'<div class="glossary-family-title" style="color:{color};">{html.escape(group.get("icon", "•"))} {html.escape(group_name)}</div>')
    parts.append(f'<div class="glossary-family-count">{len(metrics)} metrics</div>')
    parts.append('</div>')
    ...
    for metric in metrics:
        ...
        parts.append('<div class="glossary-row">')
        parts.append(f'<div class="glossary-label">{html.escape(label)}</div>')
        parts.append(f'<div class="glossary-def">{html.escape(metric_definition(metric))}</div>')
        parts.append(f'<div><span class="{pill_class}">{html.escape(kind)}</span></div>')
        parts.append(f'<div><span class="glossary-pill">{html.escape(adjustment_label(metric))}</span></div>')
        parts.append('</div>')
...
notes = ['<div class="glossary-note-grid">']
for title, text in GENERAL_DEFINITIONS:
    notes.append('<div class="glossary-note">')
    notes.append(f'<div class="glossary-note-title">{html.escape(title)}</div>')
    notes.append(f'<div class="glossary-note-text">{html.escape(text)}</div>')
    notes.append('</div>')
notes.append('</div>')
...
with tab_outfield:
    st.markdown('<div class="glossary-section-title">Metriche giocatori di movimento</div>', unsafe_allow_html=True)
    cols = st.columns(2)
...
with tab_gk:
    st.markdown('<div class="glossary-section-title">Metriche portieri</div>', unsafe_allow_html=True)
    cols = st.columns(2)
